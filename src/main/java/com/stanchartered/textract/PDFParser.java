...
/*
 * This file is a proof of concept to use AWS Textract for fetching PDF from S3 bucket and parsing the text from it. Textract works in two ways 
 * synchronous and asynchronous way. Synchronous way only work for IMAGE files and for PDF it has to be driven through asynchronous way. As its async 
 * AWS needs SNS/SQS combination to update the jobid which will help underlying AWS logic to track it. Textract depends on Jobid for progress check 
 * and completion which is internally created and tracked by AWS.
 * 
 * This POC focuses on getting the text which can be later used in JSON data object for storage/calls.
 * 
 * PLEASE REFER - https://docs.aws.amazon.com/textract/latest/dg/api-async-roles.html#api-async-roles-all-topics
 * 
 */
public class PDFParser {
	...
	public enum ProcessType {
		DETECTION, ANALYSIS // there are two ways DETECTION for finding data and ANALYSIS for finding blocks
							// in PDF.
	}

	public static void main(String[] args) throws Exception {

		String document = "Receipt.pdf"; // File name to be read
		String bucket = "stancbucket"; // S3 bucket name
		String roleArn = "arn:aws:iam::XXXXXXXXXXXXXX:role/AmazonTextractRole"; // ARN of IAM role which has policy to
																				// access SQS & SNS

		AWSCredentials credentials = new BasicAWSCredentials("XXXXXXXXXXXXXXXXXXXXXXXXXX",
				"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"); // Credentials of AWS User to access AWS services

		/* NOTE - SQS,SNS and Bucket must be in same AWS region */
		sns = AmazonSNSClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(credentials))
				.withRegion(Regions.US_EAST_1).build();
		sqs = AmazonSQSClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(credentials))
				.withRegion(Regions.US_EAST_1).build();
		textract = AmazonTextractClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(credentials))
				.withRegion(Regions.US_EAST_1).build();
	...
	static void CreateTopicandQueue() {
		...
		snsTopicName = "AmazonTextractStanCharTopic" + Long.toString(System.currentTimeMillis()); // AmazonTextractStanCharTopic Name of the TOPIC
																									// created in AWS,
																									// please refer link
																									// given above to
																									// create them
		...
		sqsQueueName = "AmazonTextractStanCharQueue" + Long.toString(System.currentTimeMillis()); // AmazonTextractStanCharQueue Name of the QUEUE
																									// created in AWS,
																									// please refer link
																									// given above to
																									// create them
	...
	// Displays Block information for text detection and text analysis. Commented
	// some part of the code which checks for blocks, relationship etc. It can be
	// used if more info to be taken from PDF.
	...
	private static void DisplayBlockInfo(Block block) {
		...
		if (block.getBlockType().equals("CELL")) {
			/*
			 * System.out.println("\tCell information:"); System.out.println("\t\tColumn: "
			 * + block.getColumnIndex()); System.out.println("\t\tRow: " +
			 * block.getRowIndex()); System.out.println("\t\tColumn span: " +
			 * block.getColumnSpan()); System.out.println("\t\tRow span: " +
			 * block.getRowSpan());
			 */
